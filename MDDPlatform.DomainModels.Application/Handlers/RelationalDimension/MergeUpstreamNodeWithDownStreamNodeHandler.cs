...
public class MergeUpstreamNodeWithDownStreamNodeHandler : ICommandHandler<MergeUpstreamNodeWithDownStreamNode>
{
    ...
    public MergeUpstreamNodeWithDownStreamNodeHandler(IDomainModelRepository domainModelRepository, IDomainModelService domainModelService, IMessageBroker messageBroker, IEventMapper eventMapper)
    {
    ...
    private async Task HandleModelOperationAsync(MergeUpstreamNodeWithDownStreamNode command)
    {
        ...
        foreach(var instance in downStreamNodeInstances)
        {
            ...
            foreach(var upstreamTargetInstance in upStreamTargetInstances)
            {
                ...
                {
                    if(command.CopyUpstreamNodeProperty)
                        CopyProperty(outputModel,instance.Name,instance.Type,upstreamConcept.Properties);
                    
                    if(command.CopyDownstreamNodeProperty)
                        CopyProperty(outputModel,instance.Name,instance.Type,instance.Properties);


                    if(command.CopyUpsteramNodeOperation)
                        CopyOperation(outputModel,instance.Name,instance.Type,upstreamConcept.Operations);

                    if(command.CopyDownstreamNodeOperation)
